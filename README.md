...
```
CV Project/
├── Dataset Augmentation.ipynb          # Data preprocessing and augmentation pipeline
├── Traditional Feature Studies.ipynb  # HOG, LBP, SIFT-BoW feature extraction and classification
├── Geometric Feature Studies.ipynb    # Hough Transform and RANSAC feature extraction
├── RESNET18 Studies.ipynb             # Deep learning with ResNet-18
├── CLIP Studies.ipynb                 # Zero-shot classification with CLIP
...
2. **rotation_15**: 15° rotation applied
3. **noise**: Gaussian noise (σ=25)
4. **scaling_0.8**: 0.8x uniform scaling
5. **occlusion_25**: 25% random occlusion
6. **all_combined**: All augmentations combined

**Total Experiments**: 3 training sets × 6 test sets = **18 combinations per method**
...
- Parameters: 9 orientations, 8×8 pixels per cell, 2×2 cells per block
- Feature dimension: Variable based on image size
- Best Performance: **F1-Macro: 0.9696** (SVM_RBF, original→original)

**LBP (Local Binary Pattern)**
- Parameters: P=8, R=1, uniform patterns
- Feature dimension: 10 bins (histogram)
- Best Performance: **F1-Macro: 0.3602** (Random_Forest, original→original)
...
- Best Performance: **F1-Macro: 0.8149** (SVM_RBF, original→rotation_15)
...
- Best Performance: **F1-Macro: 0.4212** (SVM_RBF, original→original)

**RANSAC**
- Robust line and circle fitting
- Features: Inlier ratios for line and circle fits
- Best Performance: **F1-Macro: 0.2312** (SVM_RBF, original→occlusion_25)

**Combined (Hough + RANSAC)**
- Concatenated features from both methods
- Best Performance: **F1-Macro: 0.4601** (MLP, original→original)
...
- Best Performance: **F1-Macro: 0.9892, Accuracy: 98.95%** (original→original)
...
|--------|---------------|--------------|-------------------|
| **ResNet-18** | 0.9892 | 98.95% | original→original |
| **HOG + SVM** | 0.9696 | 97.00% | original→original |
| **SIFT-BoW + SVM** | 0.8149 | - | original→rotation_15 |
| **CLIP (minimal)** | 0.7730 | 76.50% | original (zero-shot) |
| **Combined Geometric + MLP** | 0.4601 | 47.80% | original→original |
| **LBP + Random Forest** | 0.3602 | - | original→original |

### Robustness Analysis

#### ResNet-18 Performance Across Test Sets:
- **Original**: 98.95% accuracy
- **Rotation 15°**: 97.55% accuracy
